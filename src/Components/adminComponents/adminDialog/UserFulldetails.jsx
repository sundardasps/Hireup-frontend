...
export function UserFulldetails({userData}) {
  ...
  return (
    <>
      <p onClick={handleOpen} variant="gradient" className="cursor-pointer">
        {userData.userName}
      </p>
            ...
            <div>
             ...
             <div className="flex justify-between gap-5 p-3 w-min ">
              <Select   size="md"
                label="Skills "
                name="title">
                { userData.skills.map((value,index)=>(<Option key={index} >{value.skill}</Option>))}
              </Select>
              <Select size="md"
                label="experience"
                name="title2"> 
                <Option>
                { userData.experience.map((value,index)=>(<Option key={index} >{index}:{value}</Option>))}
              ...
              <ul className="bg-gray-100 text-gray-600 hover:text-gray-700 hover:shadow py-2 px-3  divide-y rounded shadow-sm">
                <li className="flex items-center py-1">
                  <span>Email</span>
                  <span className="ml-auto">{userData ? userData.email : ""}</span>
                </li>
                <li className="flex items-center py-1">
                  <span>Phone</span>
                  <span className="ml-auto">{userData ? userData.number : ""}</span>
                </li>
                <li className="flex items-center py-1">
                  <span>Status</span>
